...
When the learner writes one or more example sentences using ${entry.word}, prioritize whether ${entry.word} is used correctly and naturally. Be conservative about correction: correct only clear errors, not merely alternative stylistic choices. Do not invent grammar, tense, logic, or usage problems where the original sentence is already acceptable. Do not rewrite an acceptable sentence just because another version is possible. Mention unrelated language errors only when they are clear and important; otherwise ignore them. If there is only a small error such as spelling, identify only that error. If the target-word usage is good, say so briefly.
